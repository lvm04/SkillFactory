...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        /// <summary>
        /// Обновление существующего устройства полностью
        /// </summary>
        [HttpPut]
        [Route("{id}")]
        public async Task<IActionResult> FullEdit(
                            [FromRoute] Guid id,
                            [FromBody] AddDeviceRequest request)
        {
            ...
            await _devices.FullUpdateDevice(device, room);

            return StatusCode(200, $"Устройство обновлено! Имя - {device.Name}, Серийный номер - {device.SerialNumber},  Комната подключения - {room.Name}");
        }
    }
}